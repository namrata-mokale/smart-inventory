...
const SupplierDashboard = () => {
  ...
  const getGstRate = (name, category) => {
    ...
    const luxury = ["luxury", "ac", "automobile", "high-value"];
    const exempt = ["milk", "curd", "lassi", "paneer", "vegetable", "fruit", "pulse", "wheat flour", "atta", "rice", "salt", "honey", "roti", "naan", "khakhra", "chapati", "papad", "besan", "stamp", "postal", "book", "newspaper", "bangle", "earthenware", "sanitary napkin", "perishable"];
    const reduced = ["butter", "ghee", "cheese", "paneer", "namkeen", "bhujia", "jam", "jelly", "pasta", "cornflake", "oil", "shampoo", "soap", "toothpaste", "toothbrush", "detergent", "tableware", "kitchenware", "sewing machine", "consumables", "pharmaceuticals"];
    const moderate = ["juice", "computer", "watch", "clock", "umbrella", "electronics"];

    if (luxury.some(item => n.includes(item) || c.includes(item))) return 0.28;
    ...
    if (moderate.some(item => n.includes(item) || c.includes(item))) return 0.12;
                                        ...
                                        <div className="text-right flex flex-col items-end space-y-3">
                                            ...
                                            {req.has_quoted && (
                                                ...
                                                <div className="mt-1 text-[11px] text-gray-600 border-t border-gray-200/50 pt-1">
                                                  <p>GST ({(req.my_quote_gst_rate !== undefined ? (req.my_quote_gst_rate * 100) : (req.gst_rate !== undefined ? req.gst_rate * 100 : (getGstRate(req.product_name, req.category) * 100))).toFixed(0)}%): ₹{(req.my_quote_gst || 0).toFixed(2)}</p>
